...
import com.jakkagaku.mymultiplayer.util.ProtocolTesterService
...
class BtViewModel(): ViewModel() {
    ...
    val devices: MutableLiveData<List<BtDeviceInfoModel>> by lazy { MutableLiveData(emptyList()) }
    ...
    fun start() {
        if(btAvailable && adapter != null && discovering) {
            this.devices.value = adapter.bondedDevices.map { BtDeviceInfoModel(it, true) }
    ...
    private fun onFound(device: BluetoothDevice?){
        ...
        if(device != null && l != null && l.none { it.device.address == device.address }) {
            devices.value = l + BtDeviceInfoModel(device, false)
        }
    }
